...
" The hard-Spheres equation of state"
###############################################################
class HSEOS():
    def __init__(self,sigma=1.0,model='WBI'):
        self.sigma = sigma
        self.model = model

    def weigthed_densities(self,rho):
        self.n3 = rho*np.pi*self.sigma**3/6
        self.n2 = rho*np.pi*self.sigma**2
        self.n1 = rho*self.sigma/2
        self.n0 = rho

        self.phi1 = phi1func(self.n3)
        self.dphi1dn3 = dphi1dnfunc(self.n3)
        self.phi2 = phi2func(self.n3,model=self.model)
        self.dphi2dn3 = dphi2dnfunc(self.n3,model=self.model)
        self.phi3 = phi3func(self.n3,model=self.model)
        self.dphi3dn3 = dphi3dnfunc(self.n3,model=self.model)

    # the excess free enery density
    def betaf(self,rho):
        self.weigthed_densities(rho)
        return self.n0*self.phi1 + self.phi2*self.n1*self.n2 + self.phi3*self.n2**3
    
    # the excess chemical potential
    def betamu(self,rho):
        self.weigthed_densities(rho)
        dPhidn0 = self.phi1
        dPhidn1 = self.n2*self.phi2
        dPhidn2 = self.n1*self.phi2 + 3*self.n2**2*self.phi3
        dPhidn3 = self.n0*self.dphi1dn3 +(self.n1*self.n2)*self.dphi2dn3 + (self.n2**3)*self.dphi3dn3
...
xljMBWR = (0.862308,2.976218,-8.402230,0.105413,-0.856458,1.582759,0.763942,1.753173,2.798e3,-4.839e-2,0.996326,-3.698e1,2.084e1,8.305e1,-9.574e2,-1.478e2,6.398e1,1.604e1,6.806e1,-2.791e3,-6.245128,-8.117e3,1.489e1,-1.059e4,-1.131607e2,-8.867771e3,-3.986982e1,-4.689270e3,2.593535e2,-2.694523e3,-7.218487e2,1.721802e2)
...
xljMay12 = (0.8623085097507421,2.976218765822098,-8.402230115796038,0.1054136629203555,-0.8564583828174598,1.44787318813706322,-0.310267527929454501,3.26700773856663408,4402.40210429518902,0.0165375389359225696,7.42150201869250559,-40.7967106914122298,16.4537825382141350,12.8389071227935610,-1407.06580259642897,-33.2251738947705988,17.8209627529619184,-331.646081795803070,331.495131943892488,-4399.44711295106300,-3.05878673562233238,-12849.6469455607240,9.96912508326940738,-16399.8349720621627,-256.926076715047884,-14588.020393359636,88.3082960748521799,-6417.29842088150144,121.307436784732417,-4461.88332740913756,-507.183302372831804,37.2385794546305178)
...
def acoef(kTstar,model='NewMBWR'):
    if model == 'MBWR':
        ...
        xlj = xljMay12
    return [xlj[0]*kTstar+xlj[1]*np.sqrt(kTstar)+xlj[2]+xlj[3]/kTstar+xlj[4]/kTstar**2,xlj[5]*kTstar+xlj[6]+xlj[7]/kTstar+xlj[8]/kTstar**2,xlj[9]*kTstar+xlj[10]+xlj[11]/kTstar,xlj[12],xlj[13]/kTstar+xlj[14]/kTstar**2,xlj[15]/kTstar,xlj[16]/kTstar+xlj[17]/kTstar**2,xlj[18]/kTstar**2]
...
def bcoef(kTstar,model='NewMBWR'):
    if model == 'MBWR':
        ...
        xlj = xljMay12
    return [xlj[19]/kTstar**2+xlj[20]/kTstar**3,xlj[21]/kTstar**2+xlj[22]/kTstar**4,xlj[23]/kTstar**2+xlj[24]/kTstar**3,xlj[25]/kTstar**2+xlj[26]/kTstar**4,xlj[27]/kTstar**2+xlj[28]/kTstar**3,xlj[29]/kTstar**2+xlj[30]/kTstar**3+xlj[31]/kTstar**4]
...
def Gfunc(rhostar):
    ...
    G = [(1-F)/(2*gamma)]
    for i in range(1,6):
        G.append(-(F*rhostar**(2*i)-(2*i)*G[i-1])/(2*gamma))
    return G
...
###############################################################
class LJEOS():
    def __init__(self,sigma=1.0,epsilon=1.0,model='MBWR'):
        self.sigma = sigma
